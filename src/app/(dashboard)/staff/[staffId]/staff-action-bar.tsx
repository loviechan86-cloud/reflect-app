...
import {
  setStaffActive,
  resetStaffPassword,
  type ResetStaffPasswordState,
} from "../actions";

const resetInitialState: ResetStaffPasswordState = {
  error: null,
  success: false,
};
...
export function StaffActionBar({
  ...
  const boundResetPassword = resetStaffPassword.bind(null, staffId);
    ...
    <div className="mb-6 rounded-xl border border-gray-200 bg-white p-2.5 shadow-sm">
      ...
      {showReset && (
        ...
        >
          ...
          {resetState.success && (
            <p className="w-full text-xs text-green-700">Password updated.</p>
          )}
        </form>
      )}
    </div>
